...
def main(sphere_levels, rx_levels):
    ...
    mesh.refine_points(refine_pts, level=rx_levels, padding_cells_by_level=[2, 3], finalize=False)

    mesh.finalize()

    # CHECKPOINT
    print(f"{run_num} Mesh cells: {mesh.nC:,}")
    print(f"{run_num} Mesh x extent: {mesh.nodes_x[[0,-1]]/1000} km")
    print(f"{run_num} Mesh z extent: {mesh.nodes_z[[0,-1]]/1000} km")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sphere_levels", type=int, required=True)
    parser.add_argument("--rx_levels", type=int, required=True)
    args = parser.parse_args()

    main(args.sphere_levels, args.rx_levels)